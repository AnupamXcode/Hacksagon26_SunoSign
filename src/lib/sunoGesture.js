...
function detectMovement() {
  ...
  if (wristPositions.length >= 4) {
    const totalMovement = wristPositions.slice(1).reduce((sum, p, i) => sum + distance2D(p, wristPositions[i]), 0);
...
function matchesFingerPattern(states, pattern) {
  for (let i = 0; i < 5; i++) {
    if (pattern[i] !== -1 && (states[i] ? 1 : 0) !== pattern[i]) return false;
  }
  return true;
}
export function classifySunoGesture(landmarks) {
  const none = {
    gesture: 'NONE',
    confidence: 0,
    label: 'No Gesture',
...
export function getSentence(gesture) {
...
export const GESTURE_GUIDE = [{
  gesture: 'MY_NAME_IS',
  fingers: '☝️ Index only',
  movement: 'Point to chest',
  emoji: '👉'
}, {
  gesture: 'FATHER',
  fingers: '🖐️ All open',
  movement: 'Palm upright',
  emoji: '👋'
}, {
  gesture: 'MOTHER',
  fingers: '🖐️ All open',
  movement: 'Palm relaxed/down',
  emoji: '🤲'
}, {
  gesture: 'SEE_YOU_LATER',
  fingers: '🖐️ All open',
  movement: 'Wave side-to-side',
  emoji: '👋'
}, {
  gesture: 'CAT',
  fingers: '✌️ Index + Middle',
  movement: 'Slight twitch',
  emoji: '🐱'
}, {
  gesture: 'HELP',
  fingers: '☝️ Index only',
  movement: 'Static, upward',
  emoji: '🆘'
}, {
  gesture: 'REPEAT',
  fingers: '✌️ Index + Middle',
  ...
  gesture: 'MORE',
  fingers: '🤏 Dynamic',
  movement: 'Open → Close repeatedly',
  emoji: '➕'
}, {
  gesture: 'WHERE',
  fingers: '☝️ Index only',
  ...
  gesture: 'RESTROOM',
  fingers: '🤟 Index+Mid+Ring',
  movement: 'Three fingers up',
  emoji: '🚻'
}, {
  gesture: 'I_LOVE_YOU',
