...
export function getDomainWords(letter, domain) {
  const upper = letter.toUpperCase();
  return DOMAIN_WORDS[domain]?.[upper] || GENERAL_WORDS[upper] || [];
}
export function getDomainPhrases(letter, domain) {
  const upper = letter.toUpperCase();
...
export function wordToSentence(word) {
  return WORD_SENTENCES[word.toUpperCase()] || word;
}
export function isEmergencyWord(word) {
