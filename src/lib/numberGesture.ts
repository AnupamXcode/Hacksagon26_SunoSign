...
export interface NumberGestureResult {
  number: number | null;
  label: string;
  confidence: number;
...
const NUMBER_PATTERNS: [number, boolean[]][] = [
...
function isFingerOpen(landmarks: any[], tip: number, pip: number): boolean {
...
export function classifyNumber(landmarks: any[]): NumberGestureResult {
  const none: NumberGestureResult = {
    number: null, label: 'No number', confidence: 0,
