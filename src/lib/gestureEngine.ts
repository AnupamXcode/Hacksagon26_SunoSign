...
function dist3D(a: any, b: any): number {
...
function fingerCurl(landmarks: any[], mcp: number, pip: number, dip: number, tip: number): number {
...
function isFingerBent(landmarks: any[], tip: number, dip: number, pip: number): boolean {
...
export function classifyGesture(landmarks: any[]): GestureResult {
  ...
  // --- Y: Thumb + pinky open, others closed ---
  ...
  // --- L: Thumb + index form L shape (perpendicular) ---
  if (indexExt && !middleExt && !ringExt && !pinkyExt && thumbExt) {
  ...
  // --- W: Index, middle, ring extended, pinky closed, thumb closed ---
  if (indexExt && middleExt && ringExt && !pinkyExt && !thumbExt) {
    return make('W', 86);
  }
  ...
  if (indexExt && middleExt && !ringExt && !pinkyExt) {
    // R: Index and middle crossed/very close together
    if (indexMiddleDist < palmSize * 0.06) {
      return make('R', 80);
    }
    
    // U: Index and middle together parallel (close but not crossed)
    ...
    // K: Index + middle spread, thumb between/touching middle
  ...
  if (indexExt && !middleExt && !ringExt && !pinkyExt) {
    ...
    // G: Index pointing sideways (horizontal)
    ...
    // Z: Index pointing up (static approximation; ideally motion-tracked)
  ...
  // --- O: All fingertips close to thumb tip forming circle ---
  if (!indexExt && !middleExt && !ringExt && !pinkyExt) {
  ...
  if (extCount === 0) {
    ...
    if (thumbTip.y < indexPip.y && thumbTip.y > indexTip.y &&
        thumbTip.x > Math.min(landmarks[6].x, landmarks[10].x) &&
        thumbTip.x < Math.max(landmarks[6].x, landmarks[10].x)) {
      return make('T', 74);
    }

    // M: Three fingers folded over thumb (index, middle, ring tips below thumb tip)
    ...
    // A: Fist with thumb to the side (outside)
    if (thumbExt && !isThumbAcrossPalm(landmarks)) {
      return make('A', 84);
    }

    // S: Fist with thumb across (default fist)
    if (!thumbExt) {
      return make('S', 78);
    }
  ...
  // --- J: Pinky + thumb open with curve (static approximation) ---
  if (!indexExt && !middleExt && !ringExt && pinkyExt && thumbExt) {
    return make('J', 72);
  }
