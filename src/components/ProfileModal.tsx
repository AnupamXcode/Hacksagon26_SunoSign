...
export function ProfileModal({ profile, onSave, open, onClose }: ProfileModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/40 backdrop-blur-sm fade-in" onClick={onClose}>
      <div className="bg-card rounded-2xl p-6 w-full max-w-md shadow-2xl slide-up mx-4" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-4">
          {[
            { label: 'Name', key: 'name' as const, placeholder: 'Your name' },
            { label: 'Phone', key: 'phone' as const, placeholder: '+1 234 567 890' },
            { label: 'Emergency Contact', key: 'emergencyContact' as const, placeholder: 'Emergency contact number' },
